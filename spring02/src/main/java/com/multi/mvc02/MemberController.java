...
public class MemberController {
	...
	@Autowired // 이미 만들어서 준비된 MemberDAO 객체의 주소를 찾아서 
			   // 자동으로 그 주소를 아래의 변수인 dao에 넣어주세요.!!
	MemberDAO dao; // dao = 100주소;
	
	// 회원가입처리
	@RequestMapping("insert.multi")
	public void insert(MemberDTO bag, MemberDAO dao) {
		// 메서드 입력파라메터로 bag변수만 선언해주면
...
//			// 로그인 성공 --> ok.jsp
//			return "ok"; // views 아래 ok.jsp를 무조건 호출하러 감!
//		} else {
//			// 로그인 실패 --> no.jsp
//			// return "no";
//			// 실패했을 때, webapp 아래에 있는 member.jsp를 다시 호출하게 하고 싶음.
...
//		}
	...
	public String delete(String id) {
		// delete(@RequestParam("id") String writer) --> 넘어오는 id가 writer라는 변수에 들어감
		System.out.println(id + " 회원탈퇴 처리 요청됨");
